...
marker_group_original = folium.FeatureGroup(name="Tržby").add_to(m)
marker_cluster_original = MarkerCluster(icon_create_function=icon_create_function).add_to(marker_group_original)
...
for index, row in data.iterrows():
    ...
    ).add_to(marker_cluster_original)


marker_group_counts = folium.FeatureGroup(name="Počty firiem").add_to(m)
marker_cluster_counts = MarkerCluster().add_to(marker_group_counts)
...
# Add the GeoJSON data to the left side
folium.GeoJson(
    kraje_data,
    name='geojson',
...
# Save the dual map to an HTML file
m.save('company_dual_map_tn_tt.html')
